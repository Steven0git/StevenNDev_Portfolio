...
// Re-load if CSS not loaded!
(function() {
  const links: HTMLElement = document.getElementsByTagName('link');
  for (let i = 0; i < links.length; i++) {
    const link = links[i];
    if (link.rel.toLowerCase().includes('stylesheet') && link.href) {
      let href = link.href.replace(/(&|\?)rnd=\d+/, '');
      link.href = `${href}${href.includes('?') ? '&' : '?'}rnd=${new Date().valueOf()}`;
    }
  }
})();
